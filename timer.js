...
export class EssayTimer {
    ...
    init() {
        ...
        this.goblin.updateDisplay();
    }
    
    // --- NUEVAS FUNCIONALIDADES ---
    loadAndCheckDailySession() {
        const today = new Date().toISOString().slice(0, 10); // Formato YYYY-MM-DD
        const sessionData = this.db.get('dailySession');
    ...
    saveDailySession() {
        ...
        this.db.set('dailySession', sessionData);
    }
    ...
    saveTemplate() {
        ...
        if (templateName) {
            const templates = this.db.get('templates');
    ...
    loadTheme() {
        const theme = this.db.get('theme') || 'light';
        this.setTheme(theme);
    }
    toggleTheme() {
        const current = this.db.get('theme') || 'light';
    ...
    handleBackgroundUpload(e) {
        ...
        reader.onload = () => {
            ...
            this.db.set('bgImage', data);
            document.body.style.backgroundImage = `url(${data})`;
        };
        reader.readAsDataURL(file);
    }
    clearBackgroundImage() {
        this.db.remove('bgImage');
    ...
    saveState() {
        ...
        this.db.set(this.currentEssayName, state);
    }
    loadState(essayName) {
        const state = this.db.get(essayName);
        if (!state) return;
    ...
    startNewEssay() {
        ...
        let essays = this.db.get('index') || [];
        if (!essays.includes(name)) {
            essays.push(name);
            this.db.set('index', essays);
        }
